...
const { RECIPE, computeScore, makeLayout } = require('./recipe');
...
function advance(fields, now) {
  const writes = [];
  const events = [];
  if (!fields.meta) return { writes, events };
  ...
  for (const pk of pkeys) {
    const p = fields[pk];
    // Heartbeat lives in a separate hb:<id> field (written by state polls). Fall
    // back to any legacy lastSeen on the player object. A recent tap also counts,
    // so an actively-tapping player is never pruned even if a poll is delayed.
    const hb = fields['hb:' + p.id] || 0;
    const last = Math.max(hb, p.lastSeen || 0);
    const stale = now - last > STALE_MS;
    if (stale && !(meta.phase === 'playing' && p.finished)) {
      delete fields[pk];
      writes.push({ field: pk, value: null });
      if (fields['hb:' + p.id] != null) {
        delete fields['hb:' + p.id];
        writes.push({ field: 'hb:' + p.id, value: null });
      }
    }
  }
  ...
  if (meta.phase === 'lobby') {
    if (players.length === 0) {
      ...
      if (meta.deadline == null) {
        ...
        setMeta({ phase: 'playing', startedAt, deadline: null, endedAt: null, layout: makeLayout() });
        events.push({ type: 'roundStarted', players: activeKeys.length });
      }
    }
    ...
    if (allDone || timedOut) {
      ...
      events.push({
        type: 'roundFinished',
        timedOut: !!timedOut && !allDone,
        players: activeKeys.map((pk) => {
          const p = fields[pk];
          return { finishMs: p.finishMs, score: p.score, penalties: p.penalties, timedOut: !!p.timedOut };
        })
      });
    }
