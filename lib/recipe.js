...
/**
 * Build a randomized play order for one round. "Heat oil" (id 0) is anchored
 * first and "Plate and serve" (id 9) last — plating before cooking would be
 * nonsense — and the 8 ingredient/cooking steps between them are shuffled
 * (Fisher-Yates). Returns an array of recipe ids in the sequence players must
 * tap them, e.g. [0, 5, 3, 7, 2, 8, 4, 1, 6, 9]. 8! = 40,320 possible orders.
 */
function makeOrder() {
  const middle = [1, 2, 3, 4, 5, 6, 7, 8];
...
function publicRoomState(fields) {
  ...
  return { code: meta.code, phase: meta.phase, startedAt: meta.startedAt, endedAt: meta.endedAt || null, deadline: meta.deadline || null, order: meta.order || null, players, recipe: RECIPE };
}

module.exports = { RECIPE, MAX_PLAYERS, makeRoomCode, newPlayerId, makeOrder, computeScore, publicRoomState };
